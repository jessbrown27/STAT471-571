...
pacman::p_load(randomForest, tree, ISLR, rpart, rattle, pROC, partykit, ggplot2, glmnet, leaps, dplyr, keras, neuralnet, imager, ranger, tm, lubricate, nnet, data.table, wordcloud)
```
...
neg.data2.train <- data2.train[c("rating", names(neg.coef))] 
c.y <- neg.data2.train$rating
X2 <- sparse.model.matrix(rating~., data = neg.data2.train)[, -1]
set.seed(2)

c.result.lasso <- cv.glmnet(X2, c.y, alpha=.99, family="binomial")
plot(c.result.lasso)

# save results to result.lasso
saveRDS(c.result.lasso, file="data/TextMining_lasso2.RDS")
# result.lasso can be assigned back by 
# result.lasso <- readRDS("data/TextMining_lasso2.RDS"
```
```{r}
# number of non-zero words picked up by LASSO when using lambda.1se
c.coef.1se <- coef(c.result.lasso, s="lambda.1se")  
c.coef.1se <- c.coef.1se[which(c.coef.1se !=0),] 
neg.lasso.words <- rownames(as.matrix(c.coef.1se))[-1]
length(neg.lasso.words)
neg.lasso.words[1:10]
```

Next, we create a logistic regression model:
```{r}
neg_cols <- c("rating", neg.lasso.words)
neg_sub <- neg.data2.train %>% select(all_of(neg_cols))
neg.glm <- glm(rating~., family=binomial, neg_sub)
#save results
saveRDS(result.glm, file = "data/TextMining_glm2.RDS")
```

```{r}
neg.glm.coef <- coef(neg.glm)
neg.coef <- neg.glm.coef[which(neg.glm.coef < 0)]
neg.coef <- neg.coef[-1]
neg.coef.sorted <-  sort(neg.coef, decreasing = FALSE)
neg.coef.sorted[1:2]
```
...
```{r}
neg.words <- names(neg.coef.sorted)
wordcloud(neg.words[1:100], abs(neg.coef.sorted[1:100]), colors = cor.special, ordered.colors = F, min.freq = abs(neg.coef.sorted[100]))
```
...
```{r}
set.seed(1)
fit.rf <- randomForest(yelp_data.rating~., data2.train, mtry=3, ntree=50)
predicted_oob <- fit.rf$predicted
mse_oob <- mean((data2.train$yelp_data.rating - predicted_oob)^2)  
mse_oob
```

The testing error is `r mse_oob`. We ran the RF using fixed parameters of mtry=5 and and ntree=100. To tune the parameters, we would first look at the effect of ntree with a given mtry. Then we would fix that ntree and plot the OOB errors. The recommended mtry is p/3.
...
```{r}
pc <- prcomp(data2.train, scale=TRUE)
