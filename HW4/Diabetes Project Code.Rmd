...
p5 <- ggplot(readmission, aes(x = factor(age_mod), fill = age_mod)) +
  ...
  theme_minimal()
p6 <- ggplot(readmission, aes(x = factor(age_mod), fill = factor(lessThanThirty))) +
  geom_bar(stat = "count", width = 0.7, color = "black", position = position_dodge()) + 
  scale_fill_brewer(palette="Blues") + 
  labs(title = "Distribution of Patients Readmitted <30 Days for Each Age Group", fill = "Readmitted in <30 Days", x = "Age Group") + 
  theme_minimal()
...
```{r, results=FALSE}
nrow(readmission_sub %>% filter((gender == "Unknown/Invalid") | (race == "?") | (diag3_mod == "?")))
```
...
```{r}
model.4 <- glm(lessThanThirty~ num_medications + number_emergency + number_inpatient + number_diagnoses + insulin + diabetesMed + disch_disp_modified + age_mod + diag1_mod + diag2_mod + diag3_mod, family=binomial, data = readmission_sub)
```
...
plot(1-min.test.roc$specificities, min.test.roc$sensitivities,
  ...
  round(pROC::auc(min.test.roc), 4),
  " AUC(se.test) =",
  round(pROC::auc(se.test.roc), 4)),
  ylab="Sensitivities")
...
```{r}
pROC::auc(se.test.roc)
pROC::auc(min.test.roc)
```

```{r}
min.fitted.val <- predict(model_min, data.val, type="response")
pROC::auc(data.val$lessThanThirty, min.fitted.val)
```


```{r}
plot(min.test.roc$thresholds, 1-min.test.roc$specificities, col="green", pch=16,
...
```{r}
weight <- 1/2
sum(ifelse(min.fitted.val > 0.5, 1, 0) != data.val$lessThanThirty)/length(data.val$lessThanThirty)
```

```{r}
fit.final.bayes <- as.factor(ifelse(min.fitted.test > (weight/(1 + weight)), "1", "0"))
MCE.final.bayes <- (2 * sum(fit.final.bayes[data.test$lessThanThirty == "1"] != "1")
+ sum(fit.final.bayes[data.test$lessThanThirty == "0"] != "0"))/length(data.test$lessThanThirty)
MCE.final.bayes
```

```{r}
fit.final.bayes <- as.factor(ifelse(min.fitted.val > (weight/(1 + weight)), "1", "0"))
MCE.final.bayes <- (2 * sum(fit.final.bayes[data.val$lessThanThirty == "1"] != "1")
+ sum(fit.final.bayes[data.val$lessThanThirty == "0"] != "0"))/length(data.val$lessThanThirty)
MCE.final.bayes
```

```{r}
test_pred <- ifelse(min.fitted.test > (weight/(1 + weight)), 1, 0)
table(test_pred, data.test$lessThanThirty) 
```

```{r}
val_pred <- ifelse(min.fitted.val > (weight/(1 + weight)), 1, 0)
table(val_pred, data.val$lessThanThirty) 
```
