...
pacman::p_load(dplyr, ggplot2, glmnet, car, data.table, GGally, leaps)   #add your packages here
```
...
```{r}
set.seed(10)
subset <- readmission[sample(nrow(readmission),500),]
subset %>%
  select(lessThanThirty, time_in_hospital, num_lab_procedures, num_procedures, num_medications, number_outpatient, number_emergency, number_inpatient, number_diagnoses) %>%
  ggpairs()
```
