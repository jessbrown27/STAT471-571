...
model_1se <- glm(lessThanThirty~ time_in_hospital + num_medications + number_emergency + number_inpatient + number_diagnoses + insulin + diabetesMed + disch_disp_modified + age_mod + diag1_mod + diag2_mod + diag3_mod, family=binomial, data = data.train)
```

```{r}
min.fitted.test <- predict(model_min, data.test, type="response") # get the prob's
se.fitted.test <- predict(model_1se, data.test, type="response")
data.frame(min.fitted.test, se.fitted.test)[1:10, ]
```

```{r}
min.test.roc <- roc(data.test$HD, fit1.fitted.test)
```

```{r}
confusionMatrix(data = as.factor(final_model.pred.5), # predicted value
reference = train$lessThanThirty, # true results as reference
positive = levels(train$lessThanThirty)[2])
```
