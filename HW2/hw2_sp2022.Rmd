...
```{R}
esteem_data <- read.csv("data/NLSY79.csv")
esteem_data
str(esteem_data)
summary(esteem_data)
esteem_unique <- lapply(esteem_data, unique)
esteem_unique
sum(is.na(esteem_data))
```
...
```{R}
ggplot(gather(esteem87), aes(value)) + 
...
```{R}
pairs(esteem87, xlim=c(1, 4), ylim=c(1, 4), pch=16)
cor(esteem87)
```
...
```{R}
esteem_pc <- prcomp(esteem87, scale=TRUE)
esteem_pc
```
...
summary(fit.cyl.factor)
anova(fit.cyl.factor)
```
**Answer:** Cylinders here, is very significant as a categorical/factor variable. With a p-value of < 2e-16, we reject the null hypthesis that cylinders has no impact on MPG. Rather is it a significant variable, with the coefficient not equal to zero. 
