...
```
Plotting different prediction threholds versus the false positive rate notably returned a curve without a very obvious "elbow", although 0.5 or 0.6 seem to be reasonable choices. After experimenting more manually with minimizing MCE on the testing set, we found that 0.5 was the best threshold to use. 
```{r, echo = FALSE, results = FALSE}
...
```{r}
#predict.rf <- ifelse(film.rf$predicted > .5, 1, 0)
predict.rf <- round(predict(film.rf, film.test, type = "class"))
rf.test.err <- mean(film.test$rating != predict.rf)
```

Using the model and our testing data, we get a testing error of `r rf.test.err`.
...
```{r}
predicted_ensemble <- round(((predict.logreg + predict.rf) / 2), 0)
predicted_ensemble_err <- mean(film.test$rating != predicted_ensemble)
```

There is a testing error of `predicted_ensemble_err`.
...
```{r}
film.rf.val <- randomForest(rating~., film.val, mtry=3, ntree=300)
predict.rf.val <- ifelse(film.rf.val$predicted > .5, 1, 0)
rf.val.err <- mean(film.val$rating != predict.rf.val)
```


We used the validation data set to find the validation error of our final model and found that the validation error is `r rf.val.error`.
