...
We ran PCA to get some insight on the data. We find that 2 leading PCs is enough for our analysis. PC1 is a weighted average of the variables so a higher PC1 implies that the movie has higher numeric scores given to it. PC2 has loadings with different signs so a higher PC2 score implies larger `avg_vote`, `effort`, and/or `tension` scores and a lower PC2 score implies larger `humor`, `rhythm`, and/or `erotism` scores. Plotting PC1 and PC2, while clustering on genre, we find that there is one clear cluster with the `comedy` genre having low PC2 scores, which is not surprising because comedy movies should have a high `humor` score.

For the linear regression, we were determining which variables would be the best in predicting the average vote. We started with just a simple linear model to determine whether or not we could use just gender to predict the average vote. We were able to learn that the majority of the different genres were significant in predicting the average vote. We saw that most of the genres increase the average vote given we are only looking at the affect that genre has on the average score. From the analysis of variance, we determined that all together, genre was a significant variable in predicting the average vote.

We continued on with multiple linear regression. We looked at a model that included all of the variables that made sense to predict the average vote. From the summary of this model, we were able to eliminate the year variable and the specific year modulos were not significant. Continuing on, we removed that variable and were able to come to a final linear model which included the variables, genre, duration, total votes, humor, rhythm, effort, tension, and erotism. The model also followed the linear model assumptions as required. 

Next, we ran a regression tree. Our regression tree includes the variables `effort`, `genre`, `year_mod`, `duration`, and `rhythm`. In other words, according to our regression tree, these are the variables significant to predicting the average vote.

Finally, we create a new variable called `rating` which is coded 0 if the average vote is below 6 (indicating the movie is below average in vote and is a bad movie) and is coded 1 if otherwise (indicating the movie is above average in vote and is a good movie). We run logistic regression, random forest, and an ensemble model with the new `rating` variable. From our testing errors for the 3 models, we find that the random forest model had the lowest testing error and use that as our final model for predicting whether a movie is good or bad.
...
We also decided to make genre into a factor as it contains categorical variables that would be better analyzed as factors. There are 27 different genres that were recorded and we will use to analyze. 
```{r, results=FALSE}
...
```{r}
#cleaned data
#write.csv(film,"/Users/sarahhayward/Documents/GitHub/STAT471-571/Final Project/Cleaned Data.csv", row.names = FALSE)
```
...
```{r}
sort(table(film$genre))
```
Here we have displayed a table of the different genres that the movies are sorted into. There are 27 different genres and we have the movies split up between them. The genre with the mostt movies is Drama which has 11,002 different films and the one with the least is Short Movie with just 5 different films.
...
```
```{r}
summary(film$avg_vote)
```
The variable we are predicting in this report is `avg_vote`. That is the average vote that the film received from the critics and the public. From the histogram we can see that the distribution is approximately normal. This means that most of the movies received a vote around 5-7 with the median being 6 and the mean being 5.841. We can note that there were no movies that received an average vote of 0 as the minimum is 1 and we also can note that the maximum average vote was a full score of 10. 
...
Some of the further EDA and plots have been moved to the appendix.
...
Anova(fit1)
```
From the summary in the appendix Output I, we can first not that the genre "Action" is the base for the factor and is represented by the intercept. We can also see that the majority of the different genres are significant in predicting the average vote. We can see that most of the genres increase the average vote given we are only looking at the affect that genre has on the average score. There were however a few genres that decreased the average score, those being biblical, mythology, and romance. 

Further, from the summary of fit1 we can see that some of the specific genres are not significant in predicting the average vote. We see that the genres Biblical and Sport are not significant at a level of 0.1 or less. In fact, their p-values are quite large. Looking at the ANOVA, however, we can see that `genre` as a whole is significant in predicting the average vote, as it's p-value is 2.2e-16. Because of the ANOVA, appendix Output II, we can conclude that `genre` is a significant variable in predicting the average vote and thus we will continue to use it in our analysis. 
...
film_tree_sum <- summary(film_tree)
film_tree_sum$used
```

In our tree, `r length(film_tree_sum$used)` variables are used: `effort`, `genre`, `year_mod`, `duration`, and `rhythm`. These variables used in the tree are the ones that have significant predictive qualities according to the regression tree model. There are a total of `r film_tree_sum$size` terminal nodes (final predictions).
...
```{r, echo = FALSE}
film3 <- film2
boxplot(film3$avg_vote)
```
We can see that 6 is about the median of the average vote and thus we decided to split it there for the film rating. 
```{r, echo = FALSE}
film3$rating <- as.factor(ifelse(film3$avg_vote >= 6 , 1, 0))
```

Now we want to split up our data into train, test and validation sets.
For this we decided to do a 70-20-10 split. This gives us 30,168 training rows, 5,028 testing rows, and 2,515 validation rows. We also removed the `avg_vote` column from the data since the binary `rating` variable was derived using the `avg_vote` values. 

```{r, echo = FALSE, results = FALSE}
set.seed(1) 
film4 <- film3 %>% select(-avg_vote)
n <- nrow(film4)
sample.index <- sample(n, n * 0.8)
film.train <- film4[sample.index, ]
test_and_val <- film4[-sample.index, ]
n_tv <- nrow(test_and_val)
sample_tv <- sample(n_tv, n_tv * (2/3))
film.test <- test_and_val[sample_tv, ]
film.val <- test_and_val[-sample_tv, ]
dim(film.train)
dim(film.test)
dim(film.val)
```
```{r, echo = FALSE, results = FALSE}
logreg <- glm(rating ~., family=binomial, data = film.train)
Anova(logreg)
```

Using an Anova test, we were able to see that each variable included the logistic regression model was significant. Our next step was to use the ROC curve to determine the best threshold for our binary predictions:

```{r, echo = FALSE}
predict.test <- predict(logreg, film.test, type="response") 
test.roc <- roc(film.test$rating, predict.test)
plot(test.roc$thresholds, 1-test.roc$specificities, col="green", pch=16,
xlab="Threshold on Probability",
ylab="False Positive Rate",
main = "Thresholds vs. False Postive Rates")
```
Plotting different prediction threholds versus the false positive rate notably returned a curve without a very obvious "elbow", although 0.5 or 0.6 seem to be reasonable choices. After experimenting more manually with minimizing MCE on the testing set, we found that 0.5 was the best threshold to use. We also notably decided to weigh false positives and false negatives equally given that we didn't think either outcome was necessarily "worse" than the other. 
```{r, echo = FALSE, results = FALSE}
predict.logreg <- ifelse(predict.test > 0.5, 1, 0)
logreg_error = sum(predict.logreg != film.test$rating) / length(film.test$rating)
```

With our logistic regression model, we found that the testing error is `r logreg_error`.


# Random Forest

Now we want to use Random Forest for a prediction model. First we need to fine tune mtry and ntree. The recommended mtry is p/3 so in this case where p = 10, 10/3 = 3 or 4, so we decide to use mtry as 3. From mtry = 3, we now plot with ntree value 500 to see the effect of different ntree values.

```{r}
film.rf.500 <- randomForest(rating~., film.train, mtry=3, ntree=500)
plot(film.rf.500, col="red", pch=16, type="p", 
     main="Plot for 500 trees, ")
```

From the graph, we see that the error levels off at around ntree = 300, so the error with ntree = 300 is about the same as the error at ntree = 500. We decide then to use ntree = 300 for our random forest and build our model using the training data.

```{r}
film.rf <- randomForest(rating~., film.train, mtry=3, ntree=300)
plot(film.rf)
```

```{r}
predict.rf <- predict(film.rf, film.test, type = "class")
rf.test.err <- mean(film.test$rating != predict.rf)
```

Using the model calibrated on our training data to predict using our testing data, we get a testing error of `r rf.test.err`.

# Ensemble Model

Now we combine and average our logistic regression with our random forest model for an ensemble model.

```{r}
#as numeric changes predction values to 1 and 2 instead of 0 and 1 so we subtract 1
predicted_ensemble <- round(((predict.logreg + (as.numeric(predict.rf) - 1)) / 2), 0)
predicted_ensemble_err <- mean(film.test$rating != predicted_ensemble)
```

There is a testing error of `r predicted_ensemble_err`, which is good but not as low as the model just using random forest. 

# Final Model to Predict if a Movie is Good or Bad

From our 3 models (logistic regression, random forest, and the ensemble), the one with the lowest testing error is the random forest model so we decided to use that as our final model to predict if a movie is good or bad. 

```{r}
predict.rf.val <- predict(film.rf, film.val, type = "class")
rf.val.err <- mean(film.val$rating != predict.rf.val)
```


We used the validation data set to find the validation error of our final model and found that the validation error is `r rf.val.err`, so we were able to predict the quality of movies fairly well. 
