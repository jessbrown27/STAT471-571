...
```{r, echo = FALSE, results = FALSE}
sum(ifelse(predict.test > 0.5, 1, 0) != film.test$rating) / length(film.test$rating)
```
...
Now we want to use Random Forest for a tree model. First we need to fine tune mtry and ntree. The recommended mtry is p/3 so in this case where p = 10, 10/3 = 3 or 4, so we decide to use mtry as 3. From mtry = 3, we now plot with ntree value 500 to see the effect of different ntree values.

```{r}
film.rf.500 <- randomForest(avg_vote~., film2, mtry=3, ntree=500)
plot(film.rf.500, col="red", pch=16, type="p", 
     main="Plot for 500 trees, ")
```

From the graph, we see that the error levels off at around ntree = 300, so the error with ntree = 300 is about the same as the error at ntree = 500. We decide then to use ntree = 300 for our random forest.

```{r}
film.rf <- randomForest(avg_vote~., film2, mtry=3, ntree=300)
plot(film.rf)
film.rf$mse[300]
```

The testing error we get using ntree = 300 is `r film.rf$mse[300]`. The testing errors are larger because we are predicting a continuous variable. In a later section, we use these models to predict a more categorical outcome of the `avg_vote`.
...
set.seed(1) 
n <- nrow(dtms)
sample.index <- sample(n, 0.8 * n)
dtmM <- as.matrix(dtms)
dtm_df <- as.data.frame(dtmM)
dtm_df$rating <- ifelse(film3$avg_vote > 6 , 1, 0)
names(dtm_df) <- str_replace_all(names(dtm_df), '[^[:alnum:]]', "")
dtm.train <- dtm_df[sample.index, ]
dtm.test <- dtm_df[-sample.index, ]
```
```{r, echo = FALSE, results = FALSE}
y <- dtm.train$rating
X1 <- sparse.model.matrix(y ~., data = dtm.train)[, -1]
set.seed(2)
